...
def run_command(args):
    ...
    if args.command == "repo":
        ...
        asyncio.run(handle_cpg_command(args))
    else:
...
async def handle_cpg_command(args):
    """Handle CPG subcommands."""
    if not args.cpg_command:
        console.print("[yellow]Usage: repo-ctx cpg <status|query|export>[/yellow]")
        return

    if args.cpg_command == "status":
        await cpg_status(args)
    elif args.cpg_command == "query":
        await cpg_query(args)
    elif args.cpg_command == "export":
        await cpg_export(args)


async def cpg_status(args):
    """Show Joern CPG analysis status."""
    from ..analysis import CodeAnalyzer
    ...
    try:
        ...
        if args.output == "json":
            output = {
                "joern_available": analyzer.is_joern_available(),
                "joern_version": analyzer.get_joern_version(),
                "joern_languages": list(set(analyzer.get_joern_supported_languages())),
                "treesitter_languages": analyzer.get_treesitter_supported_languages(),
            }
            print(json.dumps(output, indent=2))
        elif args.output == "yaml":
            import yaml
            ...
            if analyzer.is_joern_available():
                ...
                console.print("[bold green]✓ Joern is available[/bold green]\n")
                console.print(f"[cyan]Version:[/cyan] {version or 'Unknown'}")

                console.print("\n[bold]Supported Languages (via Joern):[/bold]")
                joern_langs = sorted(set(analyzer.get_joern_supported_languages()))
                for lang in joern_langs:
                    console.print(f"  • {lang}")

                console.print("\n[bold]Supported Languages (via Tree-sitter):[/bold]")
                ts_langs = sorted(analyzer.get_treesitter_supported_languages())
                for lang in ts_langs:
                    console.print(f"  • {lang}")

                console.print("\n[bold]Available Analysis:[/bold]")
                console.print("  • Symbol extraction (functions, classes, methods)")
                console.print("  • Call graph analysis")
                console.print("  • Data flow analysis")
                console.print("  • Control flow graphs")
                console.print("  • CPGQL queries")
                console.print("  • Graph export (DOT, GraphML, Neo4j CSV)")
            else:
                console.print("[bold red]✗ Joern is not available[/bold red]\n")
                console.print("[yellow]To install Joern:[/yellow]")
                console.print("  1. Visit https://joern.io/ for installation")
                console.print("  2. Requirements: Java 19+ (OpenJDK recommended)")
                console.print("  3. Verify: [cyan]joern --version[/cyan]\n")

                console.print("[bold]Fallback: Tree-sitter Analysis[/bold]")
                console.print("Tree-sitter based analysis is available for:")
                ts_langs = sorted(analyzer.get_treesitter_supported_languages())
                for lang in ts_langs:
                    console.print(f"  • {lang}")
...
async def cpg_query(args):
    ...
    try:
        ...
        if not analyzer.is_joern_available():
            if args.output == "json":
                print(json.dumps({"status": "error", "message": "Joern is not available"}))
            else:
                console.print("[red]Error: Joern is not installed or not available[/red]")
                console.print("\n[yellow]To install Joern, visit: https://joern.io/[/yellow]")
            sys.exit(1)

        result = analyzer.run_cpg_query(args.path, args.query, "text")
...
async def cpg_export(args):
    ...
    try:
        ...
        representation = getattr(args, 'representation', 'all')
        format = getattr(args, 'format', 'dot')

        result = analyzer.export_cpg_graph(args.path, args.output_dir, representation, format)
        ...
        if args.output == "json":
            ...
            console.print("[bold green]✓ CPG Export Complete[/bold green]\n")
            console.print(f"[cyan]Output directory:[/cyan] {result['output_dir']}")
            console.print(f"[cyan]Format:[/cyan] {result['format']}")
            console.print(f"[cyan]Representation:[/cyan] {result['representation']}")
            console.print(f"[cyan]Files created:[/cyan] {result.get('file_count', 0)}")
