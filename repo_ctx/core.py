...
class RepositoryContext:
    ...
    def _init_providers(self):
        ...
        try:
            ...
            if isinstance(e, ProviderAuthError) and self.config.github_token:
                try:
                    logger.warning("GitHub token authentication failed, falling back to unauthenticated access")
                    logger.warning("         (Public repos only, with rate limits)")
                    github_url = self.config.github_url or "https://api.github.com"
                    self.providers["github"] = ProviderFactory.create_github(
                        url=github_url,
    ...
    async def index_repository(
        ...
        Args:
            ...
            analyze_code: Whether to also analyze code and extract symbols (default: True)
        ...
        # Analyze code and extract symbols if requested
        symbols_count = 0
        if analyze_code:
            await reporter.update(message="Analyzing code", detail="Extracting symbols...")
            symbols_count = await self._analyze_and_store_symbols(
                group=group,
                project=project,
        ...
        summary = f"Indexed {total_versions} version(s)"
        if analyze_code:
            summary += f", {symbols_count} symbols"
        await reporter.complete(summary, files_indexed=total_versions)
    ...
    async def _analyze_and_store_symbols(
        self,
        group: str,
        project: str,
        library_id: int,
        ...
        """
        Analyze code in a repository and store extracted symbols.

        Args:
            group: Repository group/owner
            project: Repository name
            library_id: Database library ID
            provider_type: Provider type (github, gitlab, local)
            progress: Optional progress callback

        Returns:
            Number of symbols extracted and stored
        """
        reporter = ProgressReporter(progress, "analyze_code")
        temp_dir = None
        ...
        try:
            ...
            if provider_type == "local":
                repo_path = f"{group}/{project}" if project else group
                await reporter.update(message="Analyzing local repository", detail=repo_path)
                files = analyze_local_directory(repo_path, analyzer)
            else:
                # Clone repo to temp directory for analysis
                await reporter.update(message="Cloning repository", detail="For code analysis...")
                clone_url = get_clone_url(provider_type, group, project, self.config)
                temp_dir = clone_repo_to_temp(clone_url)
                await reporter.update(message="Analyzing code", detail=temp_dir)
                files = analyze_local_directory(temp_dir, analyzer)

            if not files:
                await reporter.update(message="No code files found", detail="Skipping analysis")
                return 0

            # Analyze all files
            await reporter.update(message=f"Analyzing {len(files)} files", detail="Extracting symbols...")
            analysis_results = analyzer.analyze_files(files)
            symbols = analyzer.aggregate_symbols(analysis_results)

            if not symbols:
                await reporter.update(message="No symbols extracted", detail="")
                return 0

            # Store symbols in database
            await reporter.update(message=f"Storing {len(symbols)} symbols", detail="Saving to database...")
            ...
            await reporter.update(message="Analysis failed", detail=str(e))
            # Don't fail the entire indexing, just log the error
