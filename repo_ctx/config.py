...
class QdrantConfig(BaseModel):
    ...
    collection_prefix: str = "repo_ctx"

    @classmethod
    def from_env(cls) -> "QdrantConfig":
        """Load QdrantConfig from environment variables."""
        return cls(
            url=os.getenv("QDRANT_URL", ":memory:"),
            api_key=os.getenv("QDRANT_API_KEY"),
            collection_prefix=os.getenv("QDRANT_COLLECTION_PREFIX", "repo_ctx"),
        )
...
class Neo4jConfig(BaseModel):
    ...
    def from_env(cls) -> "Neo4jConfig":
        """Load Neo4jConfig from environment variables."""
        in_memory_str = os.getenv("NEO4J_IN_MEMORY", "true")
        in_memory = in_memory_str.lower() in ("true", "1", "yes")
        return cls(
...
class GenAIConfig(BaseModel):
    """Configuration for GenAI/LLM services.

    Uses litellm for multi-provider support.

    Attributes:
        model: LLM model name (e.g., "gpt-5-mini", "claude-3-opus").
        embedding_model: Embedding model name.
        api_key: API key for the LLM provider.
        base_url: Optional base URL for the API.
        enabled: Whether GenAI features are enabled.
    """
    ...
    def from_env(cls) -> "GenAIConfig":
        """Load GenAIConfig from environment variables."""
        enabled_str = os.getenv("GENAI_ENABLED", "false")
...
class StorageConfig(BaseModel):
    """Unified storage configuration for all databases.

    Attributes:
        content_db_path: Path to SQLite content database.
        qdrant: Qdrant vector database configuration.
        neo4j: Neo4j graph database configuration.
    """
    model_config = ConfigDict(arbitrary_types_allowed=True)

    content_db_path: str = "~/.repo-ctx/context.db"
    ...
    def from_env(cls) -> "StorageConfig":
        ...
        return cls(
            content_db_path=os.getenv(
                "GIT_CONTEXT_STORAGE_PATH",
                os.getenv("STORAGE_PATH", "~/.repo-ctx/context.db")
            ),
...
class Config(BaseModel):
    """Main configuration for repo-ctx.
    ...
    Attributes:
        ...
        storage_path: Path to content database (legacy, use storage.content_db_path).
        storage: Unified storage configuration for all databases.
        genai: GenAI/LLM configuration.
    """
    ...
    # Storage (legacy field for backward compatibility)
