...
# These are internal artifacts and should be filtered from output
TEMP_FILE_PATTERN = re.compile(r'^(.*[/\\])?tmp[a-z0-9_]+\.(js|ts|jsx|tsx|mjs)$', re.IGNORECASE)


def is_temp_file_path(file_path: str) -> bool:
    """Check if a file path is a temporary file from Joern analysis.

    Joern's JavaScript/TypeScript frontend (jssrc2cpg) creates temporary
    files during transpilation. These should not appear in reports.

    Args:
        file_path: File path to check.

    Returns:
        True if path appears to be a temporary file.
    """
    if not file_path:
        return False

    # Check for <transpiled> placeholder (from mapper normalization)
    if file_path.startswith('<transpiled>'):
        return True

    # Check for temp file patterns
    if TEMP_FILE_PATTERN.match(file_path):
        return True

    # Check for /tmp/ paths with tmp prefix
    import os
    basename = os.path.basename(file_path)
    if file_path.startswith('/tmp/') and basename.startswith('tmp'):
        return True

    return False


# Internal artifact names from Joern CPG that should not appear in reports
INTERNAL_ARTIFACT_NAMES = {
    "ANY", "<module>", "<init>", "<clinit>", "<global>",
    "<lambda>", "<listcomp>", "<dictcomp>", "<setcomp>", "<genexpr>",
}


def is_internal_artifact_symbol(symbol: Symbol) -> bool:
    """Check if a symbol is a Joern internal artifact.

    These are CPG internal constructs that should not appear in
    code analysis reports as they are not real code symbols.

    Args:
        symbol: Symbol to check.

    Returns:
        True if the symbol is an internal artifact.
    """
    name = symbol.name
    ...
    # Check exact matches
    if name in INTERNAL_ARTIFACT_NAMES:
        return True
...
def is_valid_symbol(symbol: Symbol) -> bool:
    """Check if a symbol is valid for inclusion in reports.

    A valid symbol must:
    - Have a non-empty name
    - Not be a Joern internal artifact
    - Not be from a temp file (unless it has other valid metadata)

    Args:
        symbol: Symbol to check.

    Returns:
        True if the symbol should be included in reports.
    """
    # Must have a name
    if not symbol.name:
        return False

    # Must not be an internal artifact
    if is_internal_artifact_symbol(symbol):
        return False

    # If it has no file path AND is a known internal pattern, skip it
    if not symbol.file_path or symbol.file_path in ("", "N/A", "unknown"):
        # Symbols without file paths are usually internal artifacts
        # But allow them if they have meaningful metadata
        if symbol.line_start and symbol.line_start > 0:
            return True  # Has line number, probably valid
        return False
...
class CodeAnalysisReport:
    ...
    def __init__(self, symbols: List[Symbol], dependencies: Optional[List[Dict[str, Any]]] = None,
        ...
        """
        # Always filter out invalid symbols (Joern internal artifacts, etc.)
        valid_symbols = [s for s in symbols if is_valid_symbol(s)]

        if exclude_tests:
            self.symbols = [s for s in valid_symbols if not is_test_symbol(s)]
    ...
    def _generate_modules_section(self) -> Optional[str]:
        ...
        # Collect unique files, filtering out temp file artifacts
        files: Dict[str, Dict[str, int]] = {}
        temp_file_counts = {"classes": 0, "functions": 0, "methods": 0, "interfaces": 0, "enums": 0}
        temp_file_count = 0

        for s in self.symbols:
            file_path = s.file_path or "unknown"

            # Check if this is a temp file artifact from Joern
            if is_temp_file_path(file_path):
                temp_file_count += 1
                if s.symbol_type == SymbolType.CLASS:
                    temp_file_counts["classes"] += 1
                elif s.symbol_type == SymbolType.FUNCTION:
                    temp_file_counts["functions"] += 1
                elif s.symbol_type == SymbolType.METHOD:
                    temp_file_counts["methods"] += 1
                elif s.symbol_type == SymbolType.INTERFACE:
                    temp_file_counts["interfaces"] += 1
                elif s.symbol_type == SymbolType.ENUM:
                    temp_file_counts["enums"] += 1
                continue  # Skip temp files from the main table
        ...
        if len(files) <= 1 and temp_file_count == 0:
            return None
