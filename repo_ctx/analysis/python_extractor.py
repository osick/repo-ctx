...
class PythonExtractor:
    ...
    def __init__(self):
        ...
        self.current_file = ""
        self.current_code = ""
        self.current_code_bytes = b""  # Byte version for correct offset extraction

    def extract(self, code: str, file_path: str) -> tuple[List[Symbol], List[Dependency]]:
        """
        Extract all symbols and dependencies from Python code.
        ...
        symbols = self._extract_symbols_from_node(root, file_path)
        dependencies = self._extract_dependencies_from_node(root, file_path, symbols)

        return symbols, dependencies

    def _extract_symbols_from_node(self, node: Node, file_path: str, parent_class: Optional[str] = None) -> List[Symbol]:
        """Recursively extract symbols from a node."""
        symbols = []
        for child in node.children:
            if child.type == "function_definition":
                symbols.append(self._parse_function(child, file_path, parent_class))
            elif child.type == "class_definition":
                class_symbol, methods = self._parse_class(child, file_path, parent_class)
                symbols.append(class_symbol)
                symbols.extend(methods)
            elif child.type == "decorated_definition":
                # A decorator can wrap a function or a class
                symbols.extend(self._extract_symbols_from_node(child, file_path, parent_class))
        return symbols

    def _extract_dependencies_from_node(self, node: Node, file_path: str, symbols: List[Symbol]) -> List[Dependency]:
        """Recursively extract dependencies from a node."""
        dependencies = []
        # Extract imports
        for child in node.children:
            if child.type == "import_statement":
                for subchild in child.children:
                    if subchild.type == "dotted_name":
                        dependencies.append(Dependency(
                            source=file_path,
                            target=self._get_node_text(subchild),
                ...
                if module_node:
                    dependencies.append(Dependency(
                        source=file_path,
                        target=self._get_node_text(module_node),
                        dependency_type="import",
                        file_path=file_path,
                        line=child.start_point[0] + 1,
                    ))
        ...
        # Extract calls
        func_symbols = [s for s in symbols if s.symbol_type in (SymbolType.FUNCTION, SymbolType.METHOD)]
        for func_symbol in func_symbols:
            func_node = self._find_symbol_node(node, func_symbol)
            if func_node:
                calls = []
    ...
    def extract_symbols(self, code: str, file_path: str) -> List[Symbol]:
        ...
        """
        symbols, _ = self.extract(code, file_path)
        return symbols
    ...
    def _get_node_text(self, node: Node) -> str:
        ...
        Tree-sitter returns byte offsets, so we must extract from the byte
        representation and decode back to string. This handles UTF-8 multi-byte
        ...
        return self.current_code_bytes[start_byte:end_byte].decode("utf8")
