...
import pytest
from repo_ctx.analysis.report_generator import (
    CodeAnalysisReport,
    is_internal_artifact_symbol,
    is_valid_symbol,
)
from repo_ctx.analysis.models import Symbol, SymbolType


class TestInternalArtifactDetection:
    """Test internal artifact detection for symbols."""

    def test_any_is_internal_artifact(self):
        """ANY is Joern's root type and should be filtered."""
        symbol = Symbol(
            name="ANY",
        ...
        assert is_internal_artifact_symbol(symbol) is True

    def test_module_is_internal_artifact(self):
    ...
    def test_init_is_internal_artifact(self):
    ...
    def test_regular_class_not_internal(self):
        """Regular class names should not be filtered."""
        symbol = Symbol(
            name="UserService",
            symbol_type=SymbolType.CLASS,
        ...
        assert is_internal_artifact_symbol(symbol) is False

    def test_metaclass_is_internal(self):
...
class TestIsValidSymbol:
    """Test symbol validation for report inclusion."""

    def test_valid_symbol_with_file_path(self):
        """Symbols with file paths are valid."""
        symbol = Symbol(
            name="MyClass",
            symbol_type=SymbolType.CLASS,
    ...
    def test_internal_artifact_is_invalid(self):
        """Internal artifacts should be invalid."""
        symbol = Symbol(
            name="ANY",
            symbol_type=SymbolType.CLASS,
            file_path="N/A",
    ...
    def test_symbol_without_file_path_and_no_line_invalid(self):
        """Symbols without file path and no line are invalid."""
        symbol = Symbol(
            name="SomeClass",
            symbol_type=SymbolType.CLASS,
            file_path="",
    ...
    def test_symbol_without_file_path_but_with_line_valid(self):
        """Symbols without file path but with line number are valid."""
        symbol = Symbol(
            name="SomeClass",
            symbol_type=SymbolType.CLASS,
            file_path="",
            line_start=10,  # Has meaningful line number
        )
        assert is_valid_symbol(symbol) is True

    def test_empty_name_is_invalid(self):
        """Empty symbol names are invalid."""
        symbol = Symbol(
            name="",
            symbol_type=SymbolType.CLASS,
            file_path="test.py",
...
class TestReportFiltersInternalArtifacts:
    """Test that CodeAnalysisReport filters internal artifacts."""

    def test_filters_any_type(self):
        """Report should filter out ANY type symbols."""
        symbols = [
            Symbol(name="ANY", symbol_type=SymbolType.CLASS, file_path="N/A", line_start=1),
            Symbol(name="RealClass", symbol_type=SymbolType.CLASS, file_path="src/real.py", line_start=5),
        ]
        report = CodeAnalysisReport(symbols)

        # Should only have RealClass
        assert len(report.symbols) == 1
        assert report.symbols[0].name == "RealClass"

    def test_filters_module_type(self):
        """Report should filter out <module> symbols."""
        symbols = [
            Symbol(name="<module>", symbol_type=SymbolType.CLASS, file_path="", line_start=1),
            Symbol(name="<module>", symbol_type=SymbolType.FUNCTION, file_path="", line_start=1),
            Symbol(name="real_function", symbol_type=SymbolType.FUNCTION, file_path="main.py", line_start=10),
        ]
        report = CodeAnalysisReport(symbols)

        # Should only have real_function
        assert len(report.symbols) == 1
        assert report.symbols[0].name == "real_function"

    def test_filters_symbols_without_file_and_line(self):
        """Report should filter symbols with no file path and no line."""
        symbols = [
            Symbol(name="GarbageClass", symbol_type=SymbolType.CLASS, file_path="N/A", line_start=0),
            Symbol(name="unknown", symbol_type=SymbolType.CLASS, file_path="unknown", line_start=0),
            Symbol(name="ValidClass", symbol_type=SymbolType.CLASS, file_path="valid.py", line_start=15),
        ]
        report = CodeAnalysisReport(symbols)

        # Should only have ValidClass
        assert len(report.symbols) == 1
        assert report.symbols[0].name == "ValidClass"
...
class TestCodeAnalysisReportIntegration:
    ...
    def test_with_code_analyzer(self):
        ...
        # Use tree-sitter for in-memory code analysis (Joern requires real files)
...
def helper_function():
    pass
"""
        # analyze_file returns a tuple (symbols, dependencies)
        symbols, dep_objects = analyzer.analyze_file(code, "service.py")

        # Convert Dependency objects to dicts for CodeAnalysisReport
        deps = [
            {
