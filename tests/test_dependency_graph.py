...
class TestDependencyGraphWithCodeAnalyzer:
    ...
    def setup_method(self):
        ...
        # Force tree-sitter to avoid Joern dependency
        self.analyzer = CodeAnalyzer(use_treesitter=True)
...
def get_animal(kind):
    ...
    return Cat()
"""
        symbols, dependencies = self.analyzer.analyze_file(code, "animals.py")
...
"""
        symbols, dependencies = self.analyzer.analyze_file(code, "components.js")
...
"""
        ...
        for file_path, code in files.items():
            symbols, deps = self.analyzer.analyze_file(code, file_path)
...
def main():
    ...
    helper()
"""
        symbols, deps = self.analyzer.analyze_file(code, "test.py")

        # Should have call dependencies (Dependency objects now)
...
"""
        ...
        compute_calls = [d for d in call_deps if "compute" in d.source]
        assert len(compute_calls) >= 2


class TestClassDependencyEdges:
    """Test that class graphs include call edges between classes.

    This is the critical fix - class graphs were showing nodes but NO EDGES
    because _build_class_graph() was ignoring call dependencies.
    """

    def setup_method(self):
        """Set up analyzer and graph builder for each test."""
        from repo_ctx.analysis import CodeAnalyzer
        # Force tree-sitter to avoid Joern dependency in tests
        self.analyzer = CodeAnalyzer(use_treesitter=True)
        self.graph_builder = DependencyGraph()

    def test_class_graph_has_call_edges_between_classes(self):
        """Class graph MUST include edges when one class calls another class's methods."""
        code = """
class ServiceA:
    def process(self):
        b = ServiceB()
...
"""
        ...
        result = self.graph_builder.build(
            ...
            dependencies=deps,
            graph_type=GraphType.CLASS,
            graph_id="test-class-deps"
        )

        # Should have 2 class nodes
        class_nodes = [n for n in result.nodes.values() if n.node_type == "class"]
        assert len(class_nodes) == 2, f"Expected 2 class nodes, got {len(class_nodes)}"

        # CRITICAL: Should have at least 1 edge (ServiceA -> ServiceB)
        assert len(result.edges) >= 1, (
            f"Class graph has {len(result.edges)} edges - expected at least 1 call edge! "
            f"This is the bug: _build_class_graph() ignores call dependencies."
        )

        # Verify the edge is a calls edge
        call_edges = [e for e in result.edges if e.relation == "calls"]
        assert len(call_edges) >= 1, (
            f"Expected at least 1 'calls' edge, got {len(call_edges)}. "
            f"Edges found: {[(e.source, e.target, e.relation) for e in result.edges]}"
        )

    def test_class_graph_excludes_self_calls(self):
        """Self-calls within same class should NOT create class-level edges."""
        code = """
class MyClass:
    def method_a(self):
        self.method_b()
...
"""
        ...
        result = self.graph_builder.build(
            ...
            dependencies=deps,
            graph_type=GraphType.CLASS,
            graph_id="test-self-calls"
        )

        # Should have 1 class node
        assert len(result.nodes) == 1

        # Should have NO call edges (self-calls excluded)
        call_edges = [e for e in result.edges if e.relation == "calls"]
        assert len(call_edges) == 0, (
            f"Expected 0 call edges for self-calls, got {len(call_edges)}"
        )

    def test_class_graph_instantiation_creates_edge(self):
        """When one class instantiates another, there should be an edge."""
        code = """
class Factory:
    def create(self):
        return Product()
...
"""
        ...
        result = self.graph_builder.build(
            ...
            dependencies=deps,
            graph_type=GraphType.CLASS,
            graph_id="test-instantiation"
        )

        # Should have 2 class nodes
        assert len(result.nodes) == 2

        # Should have edge from Factory to Product (instantiation or calls)
        edges_from_factory = [e for e in result.edges if "Factory" in e.source]
        assert len(edges_from_factory) >= 1, (
            f"Expected edge from Factory to Product, got {len(edges_from_factory)}"
        )

    def test_class_graph_with_inheritance_and_calls(self):
        """Class graph should show both inheritance AND call relationships."""
        code = """
class BaseClass:
...
"""
        ...
        # Should have 3 class nodes
        assert len(result.nodes) == 3

        # Should have inheritance edge (ChildClass -> BaseClass)
        inherit_edges = [e for e in result.edges if e.relation == "inherits"]
        assert len(inherit_edges) >= 1, "Expected inheritance edge"

        # Should have call edge (ChildClass -> Helper)
        call_edges = [e for e in result.edges if e.relation == "calls"]
        assert len(call_edges) >= 1, (
            f"Expected call edge from ChildClass to Helper, got {len(call_edges)}"
        )
