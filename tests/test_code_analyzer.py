...
"""
        symbols, deps = self.analyzer.analyze_file(code, "test.js")
...
"""
        symbols, deps = self.analyzer.analyze_file(code, "test.ts")
    ...
    def test_analyze_unsupported_language(self):
        """Test analyzing unsupported file returns empty results."""
        code = "Some random text"
        symbols, deps = self.analyzer.analyze_file(code, "test.txt")
...
def process():
    os.path.join("a", "b")
"""
        # analyze_file returns (symbols, dependencies) tuple
        symbols, deps = self.analyzer.analyze_file(code, "test.py")

        assert len(deps) >= 2
        # Dependencies are Dependency objects with target attribute
...
"""
        ...
        # analyze_files returns dict of (symbols, dependencies) tuples
        symbols1, deps1 = result["module1.py"]
        symbols2, deps2 = result["module2.py"]
        symbols3, deps3 = result["utils.js"]
